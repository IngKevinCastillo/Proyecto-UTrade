      ...
      next: (res: any) => {
        if (res.estado && res.valor && Array.isArray(res.valor)) {
          ...
          if (productosFiltrados.length === 0) {
            this.toastr.info(
              'No se encontraron productos de renta con esos criterios',
              'Búsqueda y Filtros'
            );
          }
        } else {
    ...
    this.productoService.listarPorFecha(this.filtrosActivos.fecha).subscribe({
      next: (respuesta) => {
        if (respuesta.estado && Array.isArray(respuesta.valor)) {
          ...
          if (productosFiltrados.length === 0) {
            this.toastr.info('No se encontraron productos de renta para los filtros aplicados', 'Filtros');
          }
        } else {
          this.productosFiltrados = [];
          this.mostrarProductosFiltrados = true;
    ...
    this.productoService.listarRangoPrecio(
      ...
      next: (respuesta) => {
        if (respuesta.estado && Array.isArray(respuesta.valor)) {
          ...
          if (productosFiltrados.length === 0) {
            this.toastr.info('No se encontraron productos de renta en el rango de precio', 'Filtros');
          }
        } else {
          this.productosFiltrados = [];
          this.mostrarProductosFiltrados = true;
